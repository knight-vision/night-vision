...
export async function POST(req: NextRequest) {
  const { shop_id, name, price, back_type, back_value, is_default, nomination_type } = await req.json();
...
export async function PATCH(req: NextRequest) {
  const { id, name, price, back_type, back_value, is_default, nomination_type } = await req.json();
  ...
  if (nomination_type !== undefined) patch.nomination_type = nomination_type;
