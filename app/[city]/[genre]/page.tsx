...
import { getCity, getGenre } from "@/lib/cities";
import { notFound } from "next/navigation";
import type { Metadata } from "next";

export const dynamic = "force-dynamic";

type Props = { params: { city: string; genre: string } };
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${city.name}市内の${genre.name}情報を掲載。料金・雰囲気・キャスト情報とともに紹介。`,
    alternates: { canonical: `https://www.night-vision.jp/${city.key}/${genre.key}` },
  };
}
    ...
    <>
      ...
      <main style={{ maxWidth: 680, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, color: "var(--accent)", letterSpacing: "0.15em", marginBottom: 6 }}>
            {genre.englishLabel}
          </div>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13, lineHeight: 1.7 }}>
            {city.name}市内の{genre.name} {shops.length}件を掲載。
          </p>
        </div>
