...
export default function CastMyRecordTab({ shopId, castId, castName }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const mySlips = Array.isArray(slipData) ? slipData : [];
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16 }}>
        {[{ label:"出勤日数", value:totalDays, unit:"日" },{ label:"担当伝票", value:totalSlips, unit:"枚" }].map(s => (
      ...
      {loading ? <div style={{ textAlign:"center" as const, color:"var(--text-muted)", padding:24 }}>読み込み中...</div> : (<>
            ...
            {calDays.map((date, idx) => {
              ...
              return (
                <button key={date} onClick={() => setSelectedDate(isSelected ? null : date)} style={{
                  ...
                  background: isSelected ? "linear-gradient(135deg,var(--accent),var(--accent2))"
        ...
        {selectedDate && (
          <div>
            <div style={{ fontSize:15, fontWeight:700, color:"var(--text-primary)", marginBottom:12 }}>{fmtDateLong(selectedDate)}</div>

            {/* シフト */}
            {dayShift ? (
              <div style={{ ...sec, background:"var(--online)15", borderColor:"var(--online)44" }}>
                <div style={{ fontSize:11, color:"var(--online)", fontWeight:700, marginBottom:4 }}>✅ 出勤確定</div>
                <div style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)" }}>{dayShift.start_time} 〜 {dayShift.end_time}</div>
              </div>
            ) : (
              <div style={{ ...sec }}>
            ...
            {/* 担当伝票 */}
            <div style={{ fontSize:12, fontWeight:700, color:"var(--text-muted)", margin:"14px 0 8px", letterSpacing:"0.08em" }}>
              担当伝票 {daySlips.length > 0 ? `(${daySlips.length}件)` : ""}
            </div>
            {daySlips.length === 0 ? (
              <div style={{ ...sec, color:"var(--text-hint)", fontSize:13, textAlign:"center" as const }}>この日の担当伝票はありません</div>
            ) : daySlips.map(slip => {
                ...
                <div key={slip.id} style={sec}>
                  ...
                  <div style={{ marginBottom:6 }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"var(--text-muted)", paddingTop:6, borderTop:"1px solid var(--border)", marginTop:4 }}>
                      <span>合計（税込）</span><span style={{ fontWeight:700 }}>¥{(slip.total||0).toLocaleString()}</span>
                    </div>
                  </div>
